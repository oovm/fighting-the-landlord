...
impl Display for Data {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        writeln!(f, "总实验数: {}\n", self.samples)?;
        writeln!(f, "- 缺牌概率: {:.3} %", self.has_miss as f64 / self.samples as f64 * 100.0)?;
        writeln!(f, "- 缺牌期望: {:.4}", self.count_miss as f64 / self.samples as f64)?;
        writeln!(f, "- 单张概率: {:.3} %", self.has_solo as f64 / self.samples as f64 * 100.0)?;
        writeln!(f, "- 单张期望: {:.4}", self.count_solo as f64 / self.samples as f64)?;
        writeln!(f, "- 对子概率: {:.3} %", self.has_pair as f64 / self.samples as f64 * 100.0)?;
        writeln!(f, "- 对子期望: {:.4}", self.count_pair as f64 / self.samples as f64)?;
        writeln!(f, "- 飞机概率: {:.3} %", self.has_trio as f64 / self.samples as f64 * 100.0)?;
        writeln!(f, "- 飞机期望: {:.4}", self.count_trio as f64 / self.samples as f64)?;
        writeln!(f, "- 炸弹概率: {:.3} %", self.has_bomb as f64 / self.samples as f64 * 100.0)?;
        writeln!(f, "- 炸弹期望: {:.4}", self.count_bomb as f64 / self.samples as f64)?;
        writeln!(f, "- 王炸概率: {:.3} %", self.has_nuke as f64 / self.samples as f64 * 100.0)?;
